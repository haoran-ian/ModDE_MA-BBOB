...
def average_x(bounds, restricted=True):
    ...
    if not restricted:
        return np.mean(array[:, 4]), np.mean(array[:, 4])
...
if __name__ == "__main__":
    ...
    if not os.path.exists("results/post_plots/restricted/"):
        os.mkdir("results/post_plots/restricted/")
    ...
    mean_positive, mean_negtive = average_x(bounds)
    if not df_restricted_positive.empty:
        ...
        title = f"Random dimension that the optima are close to\n where optima are close to the bound in {bounds} dimension"
        title += "s" if bounds > 1 else ""
        plt.title(title)
        plt.savefig(
            f"results/post_plots/restricted/{bounds}_restricted_positive.png")
        plt.cla()
    if not df_restricted_negtive.empty:
        ...
        plt.title(title)
        plt.savefig(
            f"results/post_plots/restricted/{bounds}_restricted_negtive.png")
        plt.cla()
    # if not df_unrestricted.empty:
    #     x_mean = average_x(bounds, restricted=False)
